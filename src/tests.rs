...
mod tests {
    ...
    fn register_available_name_and_query_works() {
        ...
        assert_name_owner(deps.as_ref(), "alice.flix", "alice_key");
    }
    ...
    fn register_available_name_and_query_works_with_fees() {
        ...
        // querying for name resolves to correct address
        assert_name_owner(deps.as_ref(), "alice.flix", "alice_key");
        assert_name_owner(deps.as_ref(), "bob.flix", "bob_key");
    }
    ...
    fn register_available_name_fails_with_invalid_name() {
        ...
        match execute(deps.as_mut(), mock_env(), info.clone(), msg) {
            ...
            Err(ContractError::InvalidSuffix { .. }) => {},
            Err(_) => panic!("Unknown error"),
        }
    ...
    fn transfer_works() {
        ...
        assert_name_owner(deps.as_ref(), "alice.flix", "bob_key");
    }
    ...
    fn fails_on_transfer_non_existent() {
        ...
        match res {
            Ok(_) => panic!("Must return error"),
            Err(ContractError::NameNotExists { name }) => assert_eq!(name, "alice42.flix"),
            Err(e) => panic!("Unexpected error: {:?}", e),
        }
