...
pub fn assert_sent_sufficient_coins(
    sent: &[Coin],
    required: &[Coin],
) -> Result<(), ContractError> {
    if required.iter().all(|required_coin|{
        let required_amount = required_coin.amount.u128();
        if required_amount != 0 {
            sent.iter().any(|sent_coin| sent_coin.denom == required_coin.denom && sent_coin.amount.u128() >= required_amount )
        } else { true }
    }) { Ok(()) } else { Err(ContractError::InsufficientFundsSent {  }) }
}
...
mod test {
    ...
    fn assert_sent_sufficient_coins_work(){
        ...
        match assert_sent_sufficient_coins(&[coin(10, "ujunox"), coin(10, "ucosm")], &[coin(10, "ucosm"),coin(10, "ujunox")]) {
            Ok(()) => {},
            Err(e) => panic!("Unexpected error: {:?}", e),
        }

        match assert_sent_sufficient_coins(&[coin(10, "ujunox"), coin(8, "ucosm")], &[coin(10, "ucosm"),coin(10, "ujunox")]) {
            Ok(()) => panic!("Should have raised insufficient funds error"),
            Err(ContractError::InsufficientFundsSent {  }) => {},
            Err(e) => panic!("Unexpected error: {:?}", e),
        }

        match assert_sent_sufficient_coins(&[coin(8, "ujunox"), coin(8, "ucosm")], &[coin(10, "ucosm"),coin(10, "ujunox")]) {
            Ok(()) => panic!("Should have raised insufficient funds error"),
            Err(ContractError::InsufficientFundsSent {  }) => {},
            Err(e) => panic!("Unexpected error: {:?}", e),
        }

        match assert_sent_sufficient_coins(&[coin(8, "ujunox"), coin(10, "ucosm")], &[coin(10, "ucosm"),coin(10, "ujunox")]) {
            Ok(()) => panic!("Should have raised insufficient funds error"),
            Err(ContractError::InsufficientFundsSent {  }) => {},
            Err(e) => panic!("Unexpected error: {:?}", e),
        }
